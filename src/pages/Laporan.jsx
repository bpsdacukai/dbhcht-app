...
// ── Tanda tangan — format sesuai lampiran ─────────────────────
// Kiri: PIHAK SEKRETARIAT TIM KOORDINASI
// Kanan: PIHAK PERANGKAT DAERAH
// Bawah tengah: Mengetahui a.n Ketua
function TandaTangan({ ps, po, kota }) {
  const s0 = (ps || []).find(p => p.nama) || {}
  const o0 = (po || []).find(p => p.nama) || {}
  return (
    <div style={{ marginTop: 18, fontSize: 11 }}>
        ...
        <tbody>
          <tr>
            <td style={{ width: '44%', textAlign: 'center', verticalAlign: 'top' }}>
              <div style={S.bold}>PIHAK SEKRETARIAT TIM</div>
              <div style={S.bold}>KOORDINASI</div>
              <br />
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, textAlign: 'left' }}>
                <tbody>
                  <tr><td style={{ width: 70 }}>Nama</td><td>: {s0.nama || ''}</td></tr>
                  <tr><td>NIP</td><td>: {s0.jabatan || ''}</td></tr>
                  <tr><td>Tanda Tangan</td><td></td></tr>
                </tbody>
              </table>
              <div style={{ marginTop: 48, borderTop: '1px solid #000', width: '80%', margin: '48px auto 0' }}>
                {s0.nama || '(____________________________)'}
              </div>
            </td>
            <td style={{ width: '12%' }} />
            <td style={{ width: '44%', textAlign: 'center', verticalAlign: 'top' }}>
              <div style={S.bold}>PIHAK PERANGKAT DAERAH</div>
              <br />
              <br />
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, textAlign: 'left' }}>
                <tbody>
                  <tr><td style={{ width: 70 }}>Nama</td><td>: {o0.nama || ''}</td></tr>
                  <tr><td>NIP</td><td>: {o0.jabatan || ''}</td></tr>
                  <tr><td>Tanda Tangan</td><td></td></tr>
                </tbody>
              </table>
              <div style={{ marginTop: 48, borderTop: '1px solid #000', width: '80%', margin: '48px auto 0' }}>
                {o0.nama || '(____________________________)'}
              </div>
            </td>
          </tr>
        </tbody>
      ...
      <div style={{ textAlign: 'center', marginTop: 24 }}>
        ...
        <div>{kota}</div>
        <div>Sekretaris</div>
        <div style={{ marginTop: 48, borderTop: '1px solid #000', width: 220, margin: '48px auto 0' }}>
...
// Print CSS: saat cetak dari overlay, hanya #ba-doc-area yang terlihat
const PRINT_CSS = `
@media print {
  body > *:not(#simdbh-print-portal) { display: none !important; }
  #simdbh-print-portal { display: block !important; }
  #simdbh-print-portal > .print-toolbar { display: none !important; }
...
function ensurePrintCss() {
  if (document.getElementById('simdbh-prt')) return
...
// Portal overlay — isi dokumen dirender di sini saat cetak
// Semua elemen lain disembunyikan oleh @media print
function PrintPortal({ children, onClose, title }) {
  useEffect(() => { ensurePrintCss() }, [])

  // Mount portal ke body
  useEffect(() => {
    let el = document.getElementById('simdbh-print-portal')
    if (!el) {
      el = document.createElement('div')
      el.id = 'simdbh-print-portal'
      document.body.appendChild(el)
    }
    return () => {
      const e = document.getElementById('simdbh-print-portal')
      if (e) e.innerHTML = ''
    }
  }, [])
  ...
  return (
    <div style={{
      ...
      {/* Toolbar — hilang saat cetak karena class no-print + @media print */}
      <div className="no-print" style={{
        ...
        <button onClick={() => window.print()}
          style={{ background: '#52b788', color: '#fff', border: 'none', padding: '7px 20px', borderRadius: 5, cursor: 'pointer', fontWeight: 700, fontSize: '1rem' }}>
          🖨️ Cetak
        </button>
        <button onClick={onClose}
          style={{ background: 'transparent', color: '#ccc', border: '1px solid #3a5a3c', padding: '7px 14px', borderRadius: 5, cursor: 'pointer' }}>
          ✕ Tutup
        </button>
      </div>
      {/* Dokumen */}
      <div id="simdbh-print-portal" style={{ flex: 1, overflow: 'auto', background: '#d0d0d0', padding: '1.5rem' }}>
        <div className="print-toolbar no-print" style={{ display: 'none' }} />
        <div style={{ background: '#fff', boxShadow: '0 4px 24px rgba(0,0,0,.3)', display: 'inline-block', minWidth: 500 }}>
          {children}
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      {menu === 'rekap_asis' && (
        <>
          <div className="no-print" style={{ marginBottom: '.75rem', display: 'flex', gap: '.5rem' }}>
            <button className="btn btn-primary" onClick={() => window.print()}>🖨️ Cetak Rekap</button>
          </div>
          <div id="simdbh-print-portal" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 8 }}>
      ...
      {menu === 'rkp' && (
        <>
          <div className="no-print" style={{ marginBottom: '.75rem', display: 'flex', gap: '.5rem' }}>
            <button className="btn btn-primary" onClick={() => window.print()}>🖨️ Cetak RKP</button>
          </div>
          <div id="simdbh-print-portal" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 8 }}>
      ...
      {menu === 'realisasi_tw' && (
        <>
          <div className="no-print" style={{ marginBottom: '.75rem', display: 'flex', gap: '.5rem' }}>
            <button className="btn btn-primary" onClick={() => window.print()}>🖨️ Cetak Laporan</button>
          </div>
          <div id="simdbh-print-portal" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 8 }}>
      ...
      {menu === 'realisasi_s1' && (
        <>
          <div className="no-print" style={{ marginBottom: '.75rem', display: 'flex', gap: '.5rem', alignItems: 'center' }}>
            <button className="btn btn-primary" onClick={() => window.print()}>🖨️ Cetak Laporan Semester I</button>
      ...
      {menu === 'realisasi_s2' && (
        <>
          <div className="no-print" style={{ marginBottom: '.75rem', display: 'flex', gap: '.5rem', alignItems: 'center' }}>
            <button className="btn btn-primary" onClick={() => window.print()}>🖨️ Cetak Laporan Semester II</button>
